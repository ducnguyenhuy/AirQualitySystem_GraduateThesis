...
/* Important data to send to user and write to json file*/
typedef struct
{
    int addr;
    int aqi;
...
/* Data for each node, it is used to find the aqi, concentration of co, pm2_5*/
data_of_node_t gBufferNode[MAX_NUMBER_NODE];
/* Data to write to json file */
data_write_to_json_t gDataJson[MAX_NUMBER_NODE];
...
/* Parse data and store to node */
int update_to_node(LoRaMacMessageData_t macMsgData)
{
    co_pm25_t var;
    int nodeAddr;
    // Parse data
    ...
    if (index == -1)
    {
        ...
        gBufferNode[bufferNodeIndex].buff12hIndex = 0;
...
int creat_file(char *file_name)
{
    ...
    json_object_dotset_value(node, "node", json_parse_string("[]"));
    json_serialize_to_file_pretty(root, FILE_NAME);
}

int edit_data(JSON_Object *node_i, data_write_to_json_t newValue)
{
    json_object_set_number(node_i, "address", newValue.addr);
    json_object_set_number(node_i, "aqi", newValue.aqi);
    json_object_set_number(node_i, "pm2_5", newValue.pm2_5);
    json_object_set_number(node_i, "co", newValue.co);
}
...
int write_to_json_file(char *filename, data_write_to_json_t data)
{
    ...
    for (i = 0; i < lenArr; i++)
    {
        ...
        if (json_object_get_number(node_i, "address") == data.addr)
        {
...
LoRaMacSerializerStatus_t LoRaMac_SerializePacket(LoRaMacMessageData_t *mac_msg)
{
    ...
    uint16_t computedBufSize = LORAMAC_MHDR_FIELD_SIZE + LORAMAC_FHDR_DEV_ADD_FIELD_SIZE + LORAMAC_FHDR_F_CTRL_FIELD_SIZE + LORAMAC_FHDR_F_CNT_FIELD_SIZE + LORAMAC_FHDR_F_LEN_FIELD_SIZE;
...
void thread_up(void)
{
    ...
    int cr = 4;                    /* CR4 aka 4/8 by default */
...
/* -------------------------------------------------------------------------- */
/* --- THREAD 2: CALCULATE AQI AND WRITE DOWN TO FILE ---------- */

void *cal_and_write_to_file(void *arg)
{
    int i;
    data_write_to_json_t tmpDataJson;
