...
payLoadData_t parse_payload(uint8_t *FMPayLoad)
{
    float data_out[5];
    int count = 1;
    ...
    char *pTmp;
    pTmp = FMPayLoad;
    data_out[0] = atof(pTmp);
    for(int i = 0; i < strlen(FMPayLoad); i++)
    {
        if(FMPayLoad[i] != ',')
        {
            continue;
        ...
        i++;
        pTmp = &FMPayLoad[i];
        data_out[count] = atof(pTmp);
        count++;
    }
    payLoadData_t payLoadData;
    payLoadData.temp = data_out[0];
    payLoadData.humi = data_out[1];
    payLoadData.co2 = data_out[2];
    payLoadData.longitude = data_out[3];
    payLoadData.latitude = data_out[4];
...
/* ---------------------------------------- Parse payload: temp, humi, co2, longitude, latitude ---------------------------------------------------- */
            payLoadData_t payLoadData;

            payLoadData = parse_payload(macMsgData.FRMPayload);
            printf("Temp:       %f\n", payLoadData.temp);
            printf("Humi:       %f\n", payLoadData.humi);
            printf("CO2:        %f\n", payLoadData.co2);
